...
for fetch_line in split(readstring(fetch_result), "\n")
    date_created = match(r"CRDT- (([0-9]{4})\/([0-9]{2})\/([0-9]{2}))", fetch_line) #TA comment:use CRDT other than DA
    if date_created != nothing
        #println("Date Created: $(date_created[1])")
        push!(date_array, date_created[1])
        #TA comment:create  new array to store the year
        push!(year_array, date_created[2])
    end
end
for fetch_line in split(readstring(fetch_result), "\n")
...
#Summarizes the number of articles per year based on Date Created and prints the output into another file called netid_asg12_output2.txt
    # NOTE: This requires StatsBase package
    # The Julia webpage pointed me to this function
    summary = countmap(year_array) #TA comment:use year_array instead of date_array
